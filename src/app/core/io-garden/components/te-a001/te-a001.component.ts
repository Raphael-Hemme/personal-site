...
export class TeA001Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const canvSizeObj =
      this.windowSizeService.calculateCanvasSize(canvasConfig);
    ...
    const sketch = (s: any) => {
      ...
      s.draw = () => {
        ...
        this.ioStringArr = this.translateInputStringIntoIoArr(
          s,
          this.inputText
        );
  ...
  private generateLetter(s: p5): {
    horizontalBarArr: any[];
    verticalBarArr: any[];
    dotTriplet: DotTriplet;
  } {
    // New shape-data generation:
    const horizontalBarCount = s.random(0, 5);
    const dotTriplet = new DotTriplet(
      s.int(s.random(10, 91)),
      s.int(s.random(10, 91))
    );
  ...
  private generateRandomLine(s: p5, padding: number, size: number) {
    ...
    switch (
      s.int(s.random(3)) // 0: horizontal, 1: vertical, 2: diag down-right, 3: diag up-right
    ) {
      case 0: // horizontal
