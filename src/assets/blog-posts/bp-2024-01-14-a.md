...
The `selectMode` function is a remnant of previous versions and will be refactored in the future. Until then, it only checks if the user requested help, if all arguments are present and valid in order to enter the AUTO mode or if the script should enter the CLI mode. The `extractSourceAndTargetPathsFromArgs` function is a bit more complex and will be explained in a future post.
