### tl;dr
Do not try to copy an array in JavaScrip like you would copy a string. It will just assign a new variable name to the existing array in addition to its old name. Instead, use array.slice(), spread the old array into a new, empty array or loop over the old array to put all it's elements into a new one.
If you just assign a new variable name to refer to the same old array, any action performed on the new array is actually performed on the one array refered to by different names.
...
Use one of these (especially the first two options seem reasonable):
...
```javascript
const oldArray = ['a', 'b', 'c'];
const newArray = oldArray.map(entry => entry);
```
...
const oldArray = ['a', 'b', 'c'];
const newArr = [];
oldArray.forEach(entry => newArray.push(entry));
```
